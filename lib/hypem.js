...
(function() {
    ...
    Playlist.prototype.fetchData = function(type, filter, page, cb) {
        ...
        if(typeof page !== 'number' || page < 1) {
            throw new Error("Invalid Page Number Exception")
        }
        page = typeof page !== 'undefined' ? page : 1;
        var options = {
            hostname: this.BASE_URL,
            port: 443,
            path: typeof filter !== 'string'? this.BASE_PATH + "/"+type +"/" + page +"/"+this.format:
                this.BASE_PATH+"/"+type+"/"+filter+"/"+this.format+"/"+page,
            method: "GET"
        };
